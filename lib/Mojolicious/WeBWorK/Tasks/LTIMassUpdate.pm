...
sub run ($job, $userID = '', $setIDs = '') {
	...
	if ($setIDs && ref($setIDs) eq 'ARRAY' && $ce->{LTIGradeMode} eq 'homework') {
		if ($userID) {
			if (ref($setIDs) eq 'ARRAY') {
				unshift(@messages,
					$job->maketext('Updated grades via LTI for user [_1] for [_2] sets.', $userID, @$setIDs));
			} else {
				unshift(@messages,
			...
			if (ref($setIDs) eq 'ARRAY') {
				unshift(@messages, $job->maketext('Updated grades via LTI for all users for [_1] sets.', @$setIDs));
			} else {
