...
sub massUpdate ($c, $manual_update = 0, $userID = undef, $setIDs = undef) {
	...
	if ($ce->{debug_lti_grade_passback}) {
		if ($setIDs && ref($setIDs) ne 'ARRAY' && $userID && $ce->{LTIGradeMode} eq 'homework') {
			warn "LTI Mass Update: Queueing grade update for user $userID and set $setIDs.\n";
		} elsif ($setIDs && ref($setIDs) ne 'ARRAY' && $ce->{LTIGradeMode} eq 'homework') {
			warn "LTI Mass Update: Queueing grade update for all users assigned to set $setIDs.\n";
		} elsif ($setIDs && $userID && $ce->{LTIGradeMode} eq 'homework') {
			warn "LTI Mass Update: Queueing grade update for $userID for @$setIDs sets.\n";
		} elsif ($setIDs && $ce->{LTIGradeMode} eq 'homework') {
			warn "LTI Mass Update: Queueing grade update for all users for @$setIDs sets.\n";
		} elsif ($userID) {
