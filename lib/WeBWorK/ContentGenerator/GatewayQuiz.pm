...
sub can_recordAnswers {
	...
	my $submitTime =
		($Set->assignment_type eq 'proctored_gateway'
			...
			&& $Set->version_last_attempt_time()) ? $Set->version_last_attempt_time() : $timeNow;
	...
	if ($User->user_id ne $EffectiveUser->user_id) {
		...
		my $recordVersionsAsOther =
			$authz->hasPermissions($User->user_id, "record_set_version_answers_when_acting_as_student");
...
sub can_checkAnswers {
	...
	if (before($Set->open_date, $submitTime)) {
		...
		if ($attempts_per_version == -1 or $attempts_used < $attempts_per_version) {
			return (
				$authz->hasPermissions($User->user_id, "check_answers_after_open_date_with_attempts")
					&& ($authz->hasPermissions($User->user_id, "view_hidden_work")
						|| $canShowScores)
			);
		} else {
			return (
				$authz->hasPermissions($User->user_id, "check_answers_after_open_date_without_attempts")
					&& ($authz->hasPermissions($User->user_id, "view_hidden_work")
						|| $canShowScores)
			);
		}
		...
		return (
			$authz->hasPermissions($User->user_id, "check_answers_after_due_date")
				&& ($authz->hasPermissions($User->user_id, "view_hidden_work")
					|| $canShowScores)
		);
	} elsif (after($Set->answer_date, $submitTime)) {
		return (
			$authz->hasPermissions($User->user_id, "check_answers_after_answer_date")
				&& ($authz->hasPermissions($User->user_id, "view_hidden_work")
					|| $canShowScores)
		);
	}
...
sub pre_header_initialize {
	...
	#    doesn't matter
	my $maxAttempts =
		(defined($Problem) && defined($Problem->max_attempts()) && $Problem->max_attempts())
	...
	if ($isOpen && !$isClosed && !$self->{invalidSet}) {
		...
		if (!$requestedVersion) {
				...
				&& $authz->hasPermissions($userName, "create_new_set_version_when_acting_as_student"))
			{
				...
				$self->{invalidSet} =
					"User $effectiveUserName is being acted "
				...
				&& $timeNow < $set->due_date() + $grace)
			{
				if (between($set->open_date(), $set->due_date() + $grace, $timeNow)) {
					...
					$self->{invalidSet} =
						"No new  versions of this assignment"
			...
			{
				$self->{invalidSet} =
					"You have already taken all available versions of this "
...
sub nav {
	...
	if ($r->authz->hasPermissions($user, 'become_student') && $effectiveUser ne $user) {
		...
		my $setPage = $r->urlpath->newFromModule(
			__PACKAGE__, $r,
			courseID => $courseName,
			setID    => "$setName,v%s"
		);
...
sub body {
	...
	if ($self->{invalidSet} || $self->{invalidProblem}) {
		...
		if (defined($self->{invalidVersionCreation})
			&& $self->{invalidVersionCreation} == 1)
		{
			my $gwpage = $urlpath->newFromModule(
				$urlpath->module, $r,
		...
		return CGI::div(
			...
			CGI::div(
				{ class => 'mb-2' },
				$r->maketext(
					"The selected problem set ([_1]) is not a valid set for [_2][_3]:", $urlpath->arg("setID"),
					$effectiveUser,                                                     $usernote
				)
			),
		...
		foreach my $i (0 .. $#problems) {    # process each problem
			...
			if ($submitAnswers && $will{recordAnswers}) {
				...
				if ($self->{isClosed}) {
					...
					$scoreRecordedMessage[ $probOrder[$i] ] = $r->maketext(
						'Your score was not recorded because '
		...
		if ($caliper_sensor->caliperEnabled() && defined($answer_log)) {
			...
			if ($submitAnswers && $will{recordAnswers}) {
				...
				my $submitted_set_event = {
					...
					'action'    => 'Submitted',
					'profile'   => 'AssessmentProfile',
					'object'    => Caliper::Entity::problem_set($self->{ce}, $db, $setName),
					'generated' => Caliper::Entity::problem_set_attempt(
						$self->{ce}, $db, $setName, $versionNumber, $effectiveUser, $startTime, $endTime
					),
				};
	...
	my $testNounNum =
		(($set->attempts_per_version || 0) > 1)
		? $r->maketext("submission (version [_1])", $versionNumber)
		: $r->maketext("version ([_1])",            $versionNumber);
	...
	if ($will{recordAnswers}) {
		...
		if ($recdMsg) {
			...
			print CGI::strong($r->maketext("Your score on this [_1] was NOT recorded.", $testNounNum), $recdMsg),
				CGI::br();
		} else {
			...
			if ($can{showScore}) {
				print CGI::strong($r->maketext(
					"Your score on this [_1] is [_2]/[_3].", $testNoun, $attemptScore, $totPossible));
			} else {
		...
		if ($can{showScore}) {
			print CGI::start_div({ class => 'gwMessage' });
			print CGI::strong($r->maketext(
				"Your score on this (checked, not recorded) submission is [_1]/[_2].", $attemptScore,
				$totPossible
				)),
				CGI::br();
			my $recScore = wwRound(2, $recordedScore);
			...
			print CGI::div(
				...
				CGI::b(
					$r->maketext('You are out of time!') . ' '
						. (
							$set->attempts_per_version > 1
							&& $attemptNumber > 0 ? '' : $r->maketext('Press "Grade Test" now!')
						)
				)
			);
		}
	...
	if (!$can{recordAnswersNextTime} && !$canShowWork) {
		...
		print CGI::div(
			...
			? CGI::strong($r->maketext(
				'Completed results for this assignment are not available until [_1].',
				$self->formatDateTime($set->answer_date)
			))
			: CGI::strong($r->maketext('Completed results for this assignment are not available.'))
		);
	} else {
		# Problems can be shown, so print out the main form and the problems.
		...
		if ($numProbPerPage && $numPages > 1) {
			push(@cols,
				CGI::colgroup({ class => 'page' }, CGI::col({ class => 'problem' }) x $numProbPerPage) x $numPages);
			my @pages;
			for my $i (1 .. $numPages) {
				my $pn =
					$i == $pageNumber
					? $i
					: CGI::a({ href => '#', class => 'page-change-link', data_page_number => $i }, $i);
				my $class = $i == $pageNumber ? 'page active' : 'page';
				push(@pages, CGI::td({ colspan => $numProbPerPage, class => $class }, $pn));
			}
			if ($numProbPerPage == 1) {
				push(@tableRows, CGI::Tr(CGI::th({ scope => 'row' }, $r->maketext('Move to Problem:')), @pages));
			} else {
				push(
					@tableRows,
					CGI::Tr(
						CGI::th({ scope => 'row', class => 'text-nowrap' }, $r->maketext('Move to Page:')), @pages
					),
					CGI::Tr(
						CGI::th({ class => 'text-nowrap' }, $r->maketext('Jump to Problem:')),
						CGI::td({ class => 'problem' }, $probRow)
					)
				);
			}
		} else {
		...
		$jumpLinks = CGI::div(
			...
			CGI::table(
				{ class => 'gwNavigation', role => 'navigation', 'aria-label' => 'problem navigation' },
				@cols, @tableRows
			)
		);
		print $jumpLinks;
		...
		# Note: args to attemptResults are (self,) $pg, $showAttemptAnswers, $showCorrectAnswers, $showAttemptResults
		# (and-ed with $showAttemptAnswers), $showSummary, $showAttemptPreview (or-ed with zero)
		my $problemNumber           = 0;
		...
		for my $i (0 .. $#pg_results) {
			...
			if ($i >= $startProb && $i <= $endProb) {
				...
				if ($pg->{flags}->{showPartialCorrectAnswers} >= 0 && $submitAnswers) {
					...
					$resultsTable = $self->attemptResults(
						$pg, 1,
				...
				if ($self->{will}{showProblemGrader}) {
					my $problem_grader = WeBWorK::ContentGenerator::Instructor::SingleProblemGrader->new($self->r, $pg,
						$problems[ $probOrder[$i] ]);
				...
				for my $curr_field (@curr_fields) {
					for (split(/\0/, $self->{formFields}->{$curr_field} // '')) {
		...
		print CGI::div(
			{ class => 'submit-buttons-container col-12 mb-2' },
			CGI::submit({
				name  => 'previewAnswers',
			...
			? CGI::submit({
				...
				$set->attempts_per_version
				? (
					data_confirm_dialog_title   => $r->maketext('Do you want to grade this test?'),
					data_confirm_btn_text       => $r->maketext('Yes'),
					data_cancel_btn_text        => $r->maketext('No'),
					data_confirm_dialog_message => $numLeft > 1
					? $r->maketext(
						'You have [_1] submissions remaining for this test.  If you say yes, then you will have '
							. '[quant,_2,submission] remaining. Once all submissions have been used, your answers '
							. 'will be final and you will not be able to continue to work this test version.',
						$numLeft,
						$numLeft - 1
						)
					: $set->attempts_per_version > 1 ? $r->maketext(
		...
		print(CGI::hidden({ name => 'sourceFilePath', value => $r->param('sourceFilePath') }))
