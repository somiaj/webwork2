...
sub displayStudentGrades ($c) {
	...
	# Set groups.
	my (@notOpen, @open, @reduced, @closed, %allItems);
	...
	for my $setID (@allSetIDs) {
		...
		if ($setIsVersioned) {
			...
			if ($templateItem->{grade} == 0 || $item->{grade} > $templateItem->{grade}) {
				...
				for ('grade', 'grade_total', 'grade_total_right', 'problems', 'message') {
	...
	@open    = sort byUrgency @open;
	@reduced = sort byUrgency @reduced;
	@closed  = sort byUrgency @closed;
...
# Taken from ProblemSets.pm, and adjusted for location of set record.
sub byUrgency ($type) {
	my $mytime = time;
	my $A      = $a->{record};
	my $B      = $b->{record};
