...
sub initialize {
	...
	# Want to be able to determine if an update is currently running, and not allow
	# a new update to start in that case, but haven't figured out a good way to do this.
	$self->{isUpdating} = 0;    #Mojo::IOLoop->is_running;
	...
	if ($self->{isUpdating}) {
		$self->{updateMessage} =
			$r->maketext('LTI update already running. Request to update user [_1] and set [_2] aborted.',
				$prettySetID, $userID);
		return;
	}
...
sub body {
	...
	if ($self->{isUpdating}) {
		print CGI::p(
			$r->maketext(
				'An LTI grade update is currently running. Cannot start a new update until the current one has completed.'
			)
		);
	} else {
