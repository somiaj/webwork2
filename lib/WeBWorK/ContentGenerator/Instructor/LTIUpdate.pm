...
sub initialize ($c) {
		...
		if ($userID) {
			if ($nSets == 1) {
				$c->addgoodmessage($c->maketext(
					'LTI update of set [_1] for user [_2] queued.',
					format_set_name_display($setIDs[0]), $userID
				));
			} elsif ($nSets) {
				$c->addgoodmessage($c->maketext('LTI update of [_1] sets for user [_2] queued.', $nSets, $userID));
			} else {
				$c->addgoodmessage($c->maketext('LTI update of all sets for user [_1] queued.', $userID));
			}
		} elsif ($nSets == 1) {
			$c->addgoodmessage(
				$c->maketext('LTI update of set [_1] for all users queued.', format_set_name_display($setIDs[0])));
		} elsif ($nSets) {
			$c->addgoodmessage($c->maketext('LTI update of [_1] sets for all users queued.', $nSets));
		} else {
	...
	massUpdate($c, 1, $userID, @setIDs ? \@setIDs : undef);
